...
> 3. **Entitlements.** Ensure exactly these two identifiers exist (the app checks them verbatim): `chravel_explorer` and `chravel_frequent_chraveler`. Attach each product to the entitlement that matches the tier it sells — do NOT lump both Trip Passes together (the $39.99 Explorer pass must not unlock the higher Frequent tier):
>    - `chravel_explorer` → Explorer Trip Pass (`com.chravel.app.trippass.explorer`), Annual Explorer (`com.chravel.app.explorer.annual`)
>    - `chravel_frequent_chraveler` → Frequent Chraveler Trip Pass (`com.chravel.app.trippass.frequent`), Frequent Chraveler monthly (`com.chravel.app.frequent.monthly`), Annual Frequent (`com.chravel.app.frequent.annual`), Starter Pro (`com.chravel.app.pro.starter.monthly`), Growth Pro (`com.chravel.app.pro.growth.monthly`) — Pro tiers include all Frequent benefits; if the web app also uses a separate Pro entitlement, grant that too.
> 4. **Offerings.** Ensure a **"current"** Offering contains a package for every product above. CRITICAL: the native shell purchases by **RevenueCat package identifier**, not StoreKit product ID — `src/revenuecat.ts` looks up `offerings.current.availablePackages` by `PurchasesPackage.identifier` using the `packageId` the web paywall sends via the `revenuecat:purchase` bridge message. So each package's **identifier must exactly match what the web paywall passes** (e.g. RevenueCat defaults like `$rc_monthly`/`$rc_annual`, or custom IDs). If you don't know them, report the package identifiers you create so the web paywall can be aligned to them — otherwise affected "Buy" buttons return `Package not found` during review.
> 5. Report the iOS API key, the entitlement IDs, the exact **offering + package identifiers** (so I can confirm the web paywall sends the same `packageId` values), and flag any product not yet approved/available in App Store Connect.
...
> 1. **Environment Variables** for the `production` (and `preview`) environment. The production build FAILS if any of these are unset (`app.config.js` guards + the `eas-build-pre-install` hook `scripts/write-google-services.js`), even for an iOS-only build:
>    - `REVENUECAT_IOS_API_KEY = <the appl_… key from RevenueCat>`.
>    - `REVENUECAT_ANDROID_API_KEY` — use the **real** `goog_…` key. A placeholder lets an iOS build pass the guard, but `eas.json` reuses the same `production` profile for Android app-bundles and the value is baked into `extra.revenueCatAndroidApiKey` → `Purchases.configure()` on Android, so a placeholder silently breaks the next Android submission's purchases. If you must ship iOS only right now, set a placeholder BUT add a reminder to replace it with the real `goog_…` key before any Android production build.
>    - `GOOGLE_SERVICES_JSON = <full google-services.json contents>` — required for ALL production builds (the pre-install hook throws without it on `EAS_BUILD_PROFILE=production`, regardless of platform). If Firebase/Android isn't set up yet (see TODO.md), this is still needed for the iOS production build to start; provide a valid `google-services.json` JSON or complete the Firebase step first.
